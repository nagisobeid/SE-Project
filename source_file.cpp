// Nagi Obeid : Individual Source File
	...
	for (int i = 0; i < 18; i++) {
		for (int j = 0; j < 18; j++) {
			if (arr[i][j] == 's' || arr[i][j] == 'm') {
				if (arr[i][j] == 's') {
					////startX is vertical position
					pth.startX = -1*i - 0.5 + nG.yres;
					////startY is horizantal position
					pth.startY = j + 0.5;
				}
...
void updateBlnPos()
{
	...
	while(b) {
		...
		///IF ONLY ONE BALLON OR START MOVING = TRUE: CHECK
		...
		////////////////////Checking for Collision with Bullet
